...
def test_carrega_yaml_com_80_regras_minimas():
    ...
    assert len(dados["regras"]) >= 80, f"YAML tem apenas {len(dados['regras'])} regras; mínimo é 80"
...
def test_idempotente_nao_duplica_aresta(db: GrafoDB, caminho_propostas: Path):
    ...
    assert stats_1["nodes_por_tipo"].get("categoria") == stats_2["nodes_por_tipo"].get("categoria")
...
def test_mutacao_regra_yaml_substitui_aresta_antiga(db: GrafoDB, tmp_path: Path):
    ...
    categorizar_todos_items_no_grafo(db, caminho_regras=yaml_inicial, caminho_propostas=propostas)
    arestas = db.listar_edges(src_id=item_id, tipo=EDGE_TIPO_CATEGORIA_DE)
    assert len(arestas) == 1
    ...
    categorizar_todos_items_no_grafo(db, caminho_regras=yaml_mutado, caminho_propostas=propostas)
    arestas = db.listar_edges(src_id=item_id, tipo=EDGE_TIPO_CATEGORIA_DE)
    assert len(arestas) == 1, (
        f"item deve ter exatamente 1 aresta categoria_de após mutação de YAML, achei {len(arestas)}"
    )
...
def test_fallback_tambem_cria_aresta(db: GrafoDB, caminho_propostas: Path):
    ...
    item_id = upsert_item(db, "12345678000190", "2026-04-20", "999", "XYZABC DESCONHECIDO")
    stats = categorizar_todos_items_no_grafo(db, caminho_propostas=caminho_propostas)
