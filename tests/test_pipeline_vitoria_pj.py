"""Sprint 93f -- proof-of-work runtime que PJ da Vitória chega ao XLSX.  # anonimato-allow

Até Sprint 93c, o extrator Nubank cartão emitia `Nubank` literal e colapsava
PJ da Vitória em PF do André. A 93c consertou o extrator. Sprint 93f fechou  # anonimato-allow
...
2. Colisão de identificador entre PF e PJ: `_gerar_hash` em
   ...
   banco. Linha PF do André e PJ da Vitória com mesmo `(data, titulo,  # anonimato-allow
...
def test_inferir_pessoa_nubank_pf_com_parenteses_retorna_vitoria():
    ...
    mapear para Vitória. Antes da 93f caía em `Casal`."""  # anonimato-allow: fixture de matcher
    assert inferir_pessoa("Nubank (PF)", subtipo=None) == "pessoa_b"
...
def test_inferir_pessoa_nubank_pj_com_parenteses_retorna_vitoria():
    ...
    nubank_cc.py:160) deve mapear para Vitória."""  # anonimato-allow: fixture de matcher
    assert inferir_pessoa("Nubank (PJ)", subtipo=None) == "pessoa_b"


def test_inferir_pessoa_nubank_sem_sufixo_retorna_andre():
    """Cartão PF do André (path sem nubank_pj) continua emitindo `Nubank`  # anonimato-allow
    puro e deve mapear para André."""  # anonimato-allow: fixture de matcher
    assert inferir_pessoa("Nubank", subtipo=None) == "pessoa_a"
...
def test_cartao_pj_produz_tx_vitoria_e_banco_canonico(tmp_path):
    ...
    `banco_origem=Nubank (PJ)`; normalizar_transacao resolve `quem=Vitória`.  # anonimato-allow
...
def test_cc_vitoria_default_sem_subtipo_resolve_para_vitoria(tmp_path):
    """Sprint 93f -- dimensão 2 do bug: Nubank CC default da Vitória sob path  # anonimato-allow
    `vitoria/nubank_cc/` (sem sufixo _pf) não deriva subtipo. Antes da 93f
    caía em `Casal`. Fix no normalizer faz `Nubank (PF)` -> Vitória mesmo  # anonimato-allow
    ...
    for t in transacoes:
        ...
        # Dupla verificação: subtipo None E quem=pessoa_b
        # (só o fix dos sets resolve a ambiguidade)
        assert norm["quem"] == "pessoa_b", (
            f"PF CC com path sem sufixo deve resolver Vitoria via set canonico, "  # anonimato-allow
            f"got {norm['quem']!r}"
        )
...
def test_identificador_pf_pj_nao_colide_no_dedup(tmp_path):
    ...
    amount)` aparecendo em CSV PF do André E em CSV PJ da Vitória deve  # anonimato-allow
    ...
    assert tx_pf[0].identificador != tx_pj[0].identificador, (
        ...
        "entre cartão PF do André (Nubank) e PJ da Vitória (Nubank (PJ))."  # anonimato-allow
    )
