...
| 15 | JS runtime global em `shell.py::instalar_fix_sidebar_padding` afeta TODAS páginas | Preferir CSS estático escopado (`html body [data-testid="..."]`); JS só para regras que CSS não alcança. Caso: commit `928628c` (revertido em `2817706`). UX-M-04 corrige. |
...
Snapshot da auditoria honesta 2026-04-29 + sessão Onda M 2026-05-06 (detalhe em `~/.claude/plans/pure-swinging-mitten.md` e `~/.claude/plans/auditoria-honesta-da-magical-lovelace.md`).
...
- Onda T+Q+U FECHADAS (redesign tela-a-tela + shell canônico + quality gates).

**Próximo passo (Onda M — modularização do dashboard, 2026-05-06):**
- 4 sprints UX-M-01..04 + 4 sub-sprints UX-M-02.A..D em backlog/.
- Objetivo: importar tokens.css/components.css canônicos do mockup; criar fronteira `ui.py` com 9 componentes consolidados; reduzir JS runtime de 211→80 linhas.
- Ordem: M-01 || M-04 → M-02 → M-03 → (A,B,C,D paralelo).
