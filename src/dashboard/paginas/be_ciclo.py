...
def _anel_html(
    dia_atual: int, fase_atual: dict[str, Any], proxima: date | None, dias_para_proxima: int | None
) -> str:
    ...
    return f'<div class="anel-wrap">{_anel_ciclo_svg(dia_atual)}{centro}</div>'
...
def _sintomas_card_html(sintomas: list[dict[str, Any]]) -> str:
    ...
    if not sintomas:
        ...
        for s in sintomas:
            ...
            dots = "".join(
                f'<span class="{"on" if i < intensidade else ""}"></span>' for i in range(3)
            )
            linhas.append(
        ...
        corpo = "".join(linhas)
    return f'<div class="sint-card"><h3>Sintomas hoje · escala 0-3</h3>{corpo}</div>'
...
def _sintomas_skeleton_html() -> str:
    ...
    return f'<div class="sint-card"><h3>Sintomas hoje · escala 0-3</h3>{"".join(linhas)}</div>'
...
def _cruzamento_skeleton_html() -> str:
    ...
    for label in CRUZAMENTO_FASES_SKELETON:
        linhas.append(f'<div class="cruz-row"><span>{label}</span><span class="v">--</span></div>')
    nota = (
...
def _skeleton_ciclo_completo_html() -> str:
    ...
    """
    coluna_principal = (
        f'<div class="ciclo-card">{_anel_skeleton_html()}{_cards_fase_html("")}</div>'
    )
    coluna_lateral = (
    ...
    return f'<div class="ciclo-grid"><div>{coluna_principal}</div>{coluna_lateral}</div>'
...
def renderizar(
    ...
    if vault_root is None:
        st.warning(
            "Vault Bem-estar não encontrado. Configure `OUROBOROS_VAULT` para visualizar o ciclo."
        )
        return
    ...
    grid = f'<div class="ciclo-grid"><div>{coluna_principal}</div>{coluna_lateral}</div>'
