---
id: DOC-07-HISTORICO-ESCOLAR
titulo: 'Sprint DOC-07 -- Extrator: histórico escolar (PDF + grade + CR)'
status: backlog
concluida_em: null
prioridade: P2
...
# Sprint DOC-07 -- Extrator: histórico escolar (PDF + grade + CR)
...
histórico escolar (PDF + grade + CR) é documento cotidiano sem regra YAML nem extrator. Cai silenciosamente em _classificar/ ou roteamento-only.
...
2. Criar src/extractors/historico_escolar.py.
...
`make conformance-historico_escolar` retorna exit 0 (≥3 amostras 4-way verdes).

## Acceptance criteria

- Tipo em tipos_documento.yaml.
- Extrator src/extractors/historico_escolar.py com 8+ testes.
