...
PDF emitido pelo gov.br (auto-detect) é documento cotidiano sem regra YAML nem extrator. Cai silenciosamente em _classificar/ ou roteamento-only.
...
2. Criar src/extractors/govbr_pdf.py.
...
`make conformance-govbr_pdf` retorna exit 0 (≥3 amostras 4-way verdes).

## Acceptance criteria

- Tipo em tipos_documento.yaml.
- Extrator src/extractors/govbr_pdf.py com 8+ testes.
