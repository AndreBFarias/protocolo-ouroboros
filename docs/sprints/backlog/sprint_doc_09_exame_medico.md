---
id: DOC-09-EXAME-MEDICO
titulo: 'Sprint DOC-09 -- Extrator: exame médico (PDF/foto + paciente + médico + parâmetros)'
status: backlog
concluida_em: null
prioridade: P2
...
# Sprint DOC-09 -- Extrator: exame médico (PDF/foto + paciente + médico + parâmetros)
...
exame médico (PDF/foto + paciente + médico + parâmetros) é documento cotidiano sem regra YAML nem extrator. Cai silenciosamente em _classificar/ ou roteamento-only.
...
2. Criar src/extractors/exame_medico.py.
...
`make conformance-exame_medico` retorna exit 0 (≥3 amostras 4-way verdes).

## Acceptance criteria

- Tipo em tipos_documento.yaml.
- Extrator src/extractors/exame_medico.py com 8+ testes.
