...
escopo_refinado_por: pacote interno colocado em src/mobile_cache/ ao invés de src/protocolo_ouroboros/mobile_cache/ -- coerência com top_level.txt do egg-info que registra src como pacote canônico; contratos externos (paths Vault, schema JSON, atomic write, schema_version=1) preservados
entrega_real: src/mobile_cache/ (5 módulos) + tests/mobile_cache/ (33 testes), Makefile sync e mobile-cache, run.sh --full-cycle integrado, --mobile-cache standalone, ADR-0012 do Mobile como contrato cruzado
commits: 5be23a7
seguimento: nenhum (idempotência via comparação de payload entrega na MOB-bridge-2.1 só sob demanda se medirmos > 5s em produção)
---
